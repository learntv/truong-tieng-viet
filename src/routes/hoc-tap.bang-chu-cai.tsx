...
import { Check, Printer, Volume2, X } from "lucide-react";
...
/** Builds the printable alphabet PDF as a Blob. Not called from the UI — the print
 * button opens the pre-generated copy hosted on R2 (see ALPHABET_PDF_URL below).
 * Exposed on `window` in dev only (see the effect in BangChuCaiTab) so that whenever
 * ALPHABET content changes, the hosted copy can be regenerated by opening this page,
 * running `await window.__buildAlphabetPdfBlob()` in the console, and re-uploading
 * the resulting blob to `misc/bang-chu-cai-tieng-viet.pdf` in R2. */
async function buildAlphabetPdfBlob(): Promise<Blob> {
...
/** Loads a PDF (by URL) into a hidden iframe and opens the browser's native
 * print dialog on it — the "Ctrl+P" flow, letting the user choose to save
 * as PDF or print, rather than forcing a silent download. Plain resource
 * navigation like this isn't subject to CORS the way `fetch` is, so it works
 * even though the R2 bucket doesn't send CORS headers. */
...
function BangChuCaiTab() {
  ...
  useEffect(() => {
    ...
    if (import.meta.env.DEV) {
      (window as unknown as { __buildAlphabetPdfBlob: typeof buildAlphabetPdfBlob }).__buildAlphabetPdfBlob =
        buildAlphabetPdfBlob;
    }
  }, []);
